...
pub struct FramesArgs {
    ...
    ///
    /// A DIFFERENT FACT from when this command runs, and confusing the two is a
    /// check that cannot fail: regenerating from a four-day-old capture moves
    /// the ledger's `generated_at` to now and moves the capture date not at
    /// all. The staged-write bypass rule reads THIS and refuses when it is
    /// absent, because a freshness rule reading `generated_at` reports a stale
    /// reading as fresh every time. That is not hypothetical: it is how 23 of
    /// 188 routes on the subscribing estate were read against a stale capture.
    ///
...
fn frames_command(project: &vds_core::Project, store: &Store, args: &FramesArgs) -> Result<i32> {
    ...
    match &ledger.captured_at {
        ...
        None => println!(
            "  captured at:     NOT STATED. `vds proof staged_write` REFUSES its bypass rule \
             without this,\n                   because a freshness rule that reads generated_at \
             reports a stale reading\n                   as fresh every time. Pass --captured-at \
