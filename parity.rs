...
pub fn run(ctx: &ProofContext, out: &mut dyn Write) -> Result<Outcome> {
    ...
    // A precondition and not an empty pass. Parity is a comparison, and with no
    // library directory named there is no side to compare against: a run that
    // reported no violations would be reporting them about files it never
    // opened. This is the same refusal `vds register import` makes, for the same
    // reason.
    if surface.library_dirs.is_empty() {
        return Err(VdsError::precondition(
            "[surface] library_dirs is empty, so there is no component source to compare any \
             contract against and this proof did not run. Name the directories your components \
             live in, or stop claiming parity: a caller told `no violations` about a library \
             nobody opened has been told nothing.",
        ));
    }
