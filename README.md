...
**Sixteen proof kinds are a closed registry** (VDS S-7(5)), and closure is enforced by the
type system: `ProofKind` is an enum, so a kind outside the registry does not fail validation,
it fails to compile. **All sixteen are implemented** (one as a DRAFT amendment of 2026-08-03, pending enactment) (`vds proof --list`).
